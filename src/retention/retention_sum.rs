...
impl Accumulator for RetentionSum {
                ...
                ScalarValue::try_from_array(active_by_event, index)?
            {
                ...
                ) = (&v[0], &v[1])
                {
                    for (b_index, val) in born_active.iter().enumerate() {
                        total_active.push(val.clone());
                        let mut active: Vec<ScalarValue> = Vec::new();
                        for t_index in b_index..target_active.len() {
                            let x = val.bitand(target_active.get(t_index).unwrap())?;
                            if let ScalarValue::Int64(_) = x {
                                active.push(x)
                            } else {
                                unreachable!()
                            }
    ...
    fn evaluate(&self) -> Result<ScalarValue> {
        ...
        for index in 0..arr_ref.len() {
                ...
                ScalarValue::try_from_array(arr_ref, index)?
            {
                if let (
                    ...
                    ScalarValue::List(Some(target), _),
                ) = (&per_user[0], &per_user[1], &per_user[2])
                {
                    if total_cnt.is_empty() {
                        ...
                        target_cnt.resize(total.len(), ScalarValue::Int64(Some(0)));
                    }
        ...
        let final_result = ScalarValue::new_list(
            Some(vec![
                ScalarValue::new_list(Some(total_cnt), DataType::Int64),
                ScalarValue::new_list(Some(born_cnt), DataType::Int64),
                ScalarValue::new_list(Some(target_cnt), DataType::Int64),
            ]),
...
mod tests {
    ...
    #[tokio::test]
    async fn retention_sum() -> Result<()> {
        let ctx = SessionContext::new();

        ctx.register_udaf(create_retention_sum());

        let results = ctx
            .sql("select retention_sum(column2) from (values (1, [[1,1],[1,0]]), (2, [[1,0],[0,1]]))")
            .await?
            .collect()
            .await?;
